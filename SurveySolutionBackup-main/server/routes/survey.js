...
router.get('/', (req, res, next) => {
  ...
  if(!req.user) {
    ...
    let query = {displayName: req.user ? req.user.displayName : ""};
    survey.find(query,(err, survey) => {
      if (err) {
        return console.error(err);
      }
      else {
        res.render('survey/index', {title: 'Survey', survey: survey, displayName: req.user ? req.user.displayName : ""});
      }
    });
  }
});
...
router.get('/edit/:id', (req, res, next) => {
    ...
    survey.findById(id, (err, surveyEdit) => {
      ...
      {
        res.render('survey/edit', {title: 'Edit Survey', survey: surveyEdit, displayName: req.user ? req.user.displayName : ""})
      }
    });
});
  ...
  if(!req.user) {
    ...
    survey.findById(id, (err, surveyEdit) => {
      if (err) {
        ...
        res.render('survey/view', {title: 'View Survey', survey: surveyEdit, answers: false ,displayName: req.user ? req.user.displayName : ""});
      }
    });
  } else {
      ...
      if (err) {
        ...
        answers.findOne({ $and : [{userId : userId}, {surveyId : surveyId}]}, function(err, ansRes) {
          if(err) {
            ...
            res.render('survey/view', {title: 'View Survey', survey: surveyEdit, answers: ansRes, displayName: req.user ? req.user.displayName : ""});
          }
        });
      }
    });
...
router.get('/login/:id', (req, res, next) => {
  ...
  survey.findById(id, (err, surveyEdit) => {
    ...
    {
      res.render('survey/login', {title: 'Login', survey: surveyEdit , displayName: req.user ? req.user.displayName : ""})
    }
  });
